 ...
 */
public class ModelServiceImpl<T extends Model<ID>, ID extends ObjectId> implements ModelService<T, ID> {
    ...
    private Historic createHistoric(final User user) {
        return new Historic()
                .setCreatedBy(user)
                .setDtCreate(new Date());
    }

    private Historic generateHistoricUpdate(final User user, final Historic historic) {
        return historic
