...
public class AdminOwnerServiceImpl extends SecurityServiceImpl<AdminOwner> implements AdminOwnerService {
    ...
    @Autowired
    public AdminOwnerServiceImpl(final AdminOwnerRepository repository, final MongoTemplate mongoTemplate, final ApplicationEventPublisher eventPublisher, final DocumentNameConfig documentNameConfig) {
    ...
    @Override
    public void checkPrecondictionSave(final User user, final AdminOwner value) {
    ...
    @Override
    public AdminOwner save(final User user, final AdminOwner value) {
        if (value.getUserMaster() == null || value.getUserMaster().getId() == null) {
            throw new MuttleyBadRequestException(AdminOwner.class, "userMaster", "Informe um usuário válido");
        }
        if (value.getAccessPlan() == null || value.getAccessPlan().getId() == null) {
            throw new MuttleyBadRequestException(AdminOwner.class, "accessPlan", "Informe um plano de acesso");
        }
        final AdminOwner salvedOwner = super.save(user, value);
        this.eventPublisher.publishEvent(new OwnerCreateEvent(salvedOwner));
        return salvedOwner;
    }

    @Override
    public AdminOwner update(final User user, final AdminOwner value) {
        if (value.getUserMaster() == null || value.getUserMaster().getId() == null) {
            throw new MuttleyBadRequestException(AdminOwner.class, "userMaster", "Informe um usuário válido");
        }
    ...
    public void checkPrecondictionDelete(final User user, final String id) {
        throw new MuttleyBadRequestException(AdminOwner.class, "id", "Não é possível deletear um owner");
    }

    @Override
    public AdminOwner findByName(final String name) {
        final AdminOwner clienteOwner = repository.findByName(name);
